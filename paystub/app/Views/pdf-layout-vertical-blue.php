<?php /** @var array $payload */ $tokens = $tokens ?? ['primary'=>'#0f172a','accent'=>'#1d4ed8','muted'=>'#64748b']; ?>
<?php include __DIR__.'/partials/pdf-style-vertical-blue.php'; ?>
<div>
  <div class="block" style="display:flex;align-items:flex-end;justify-content:space-between;gap:16pt;">
    <?php $i=$payload['stub_index']??0; $p=$payload['periods'][$i]??null; ?>
    <div style="flex:0 0 auto;max-width:240pt;">
      <?php if (!empty($payload['branding']['logo'])): ?>
  <img src="<?= htmlspecialchars($payload['branding']['logo']) ?>" alt="Logo" style="display:block;max-width:100%;max-height:32pt;object-fit:contain;">
      <?php else: ?>
        <h1 style="margin:0;">Pay Stub</h1>
      <?php endif; ?>
    </div>
    <div style="flex:1 1 auto;text-align:right;font-size:9pt;line-height:1.3;">
      <div class="muted" style="margin:0;">Folio: <?= htmlspecialchars($payload['folio'] ?? '') ?></div>
      <div class="muted" style="margin:0;">Period: <?= htmlspecialchars($p['period_start'] ?? $p['start_date'] ?? '') ?> – <?= htmlspecialchars($p['period_end'] ?? $p['end_date'] ?? '') ?></div>
      <div class="muted" style="margin:0;">Pay: <?= htmlspecialchars($p['pay_date'] ?? '') ?></div>
    </div>
  </div>
  <div class="block">
    <table><tbody><tr>
      <td style="width:50%">
        <?php if (!empty($payload['branding']['logo'])): ?>
          <div style="margin:0 0 4pt;"><img src="<?= htmlspecialchars($payload['branding']['logo']) ?>" alt="Logo" style="max-height:24pt;max-width:100%;object-fit:contain;"></div>
        <?php endif; ?>
        <strong>Employer</strong><br><?= htmlspecialchars($payload['employer']['name'] ?? '') ?><br><?php foreach (($payload['employer']['address'] ?? []) as $line): ?><?= htmlspecialchars($line) ?><br><?php endforeach; ?></td>
