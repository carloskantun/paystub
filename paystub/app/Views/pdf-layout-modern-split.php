<?php /** @var array $payload */ $tokens = $tokens ?? ['primary'=>'#0f172a','accent'=>'#6366f1','muted'=>'#64748b']; ?>
<?php include __DIR__.'/partials/pdf-style-modern-split.php'; ?>
<div>
  <?php $i=$payload['stub_index']??0; $p=$payload['periods'][$i]??null; ?>
  <div class="block" style="display:grid;grid-template-columns:1fr 1fr;gap:8pt;align-items:start;">
    <div>
    ...
    <div style="text-align:right;">
      <div><strong>Period</strong> <?= htmlspecialchars($p['period_start'] ?? $p['start_date'] ?? '') ?> – <?= htmlspecialchars($p['period_end'] ?? $p['end_date'] ?? '') ?></div>
      <div><strong>Pay</strong> <?= htmlspecialchars($p['pay_date'] ?? '') ?></div>
    </div>
  </div>
  <div class="block" style="display:grid;grid-template-columns:1fr 1fr;gap:8pt;">
    <div>
      <table><tbody><tr>
        <td><strong>Employer</strong><br><?= htmlspecialchars($payload['employer']['name'] ?? '') ?><br><?php foreach (($payload['employer']['address'] ?? []) as $line): ?><?= htmlspecialchars($line) ?><br><?php endforeach; ?></td>
      </tr></tbody></table>
    </div>
    <div>
      <table><tbody><tr>
        <td><strong>Employee</strong><br><?= htmlspecialchars($payload['employee']['name'] ?? '') ?><br><?php foreach (($payload['employee']['address'] ?? []) as $line): ?><?= htmlspecialchars($line) ?><br><?php endforeach; ?></td>
      </tr></tbody></table>
    </div>
  </div>
