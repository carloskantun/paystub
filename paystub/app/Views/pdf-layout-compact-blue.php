...
<?php include __DIR__.'/partials/pdf-style-compact-blue.php'; ?>
<div>
  <div class="block">
    <?php $i=$payload['stub_index']??0; $p=$payload['periods'][$i]??null; ?>
    <?php if (!empty($payload['branding']['logo'])): ?>
      <div style="margin:0 0 4pt;max-width:200pt;">
        <img src="<?= htmlspecialchars($payload['branding']['logo']) ?>" alt="Logo" style="max-width:100%;max-height:32pt;object-fit:contain;">
      </div>
    <?php else: ?>
      <h1>Pay Stub</h1>
    <?php endif; ?>
    <div class="muted" style="font-size:8pt;">Folio: <?= htmlspecialchars($payload['folio'] ?? '') ?> · Period: <?= htmlspecialchars($p['period_start'] ?? $p['start_date'] ?? '') ?> – <?= htmlspecialchars($p['period_end'] ?? $p['end_date'] ?? '') ?> · Pay: <?= htmlspecialchars($p['pay_date'] ?? '') ?></div>
  </div>
  <div class="block">
    <table><tbody><tr>
      <td style="width:50%"><strong>Employer</strong><br><?= htmlspecialchars($payload['employer']['name'] ?? '') ?></td>
      <td style="width:50%"><strong>Employee</strong><br><?= htmlspecialchars($payload['employee']['name'] ?? '') ?></td>
    </tr></tbody></table>
  </div>
  ...
  <div class="block"><h2>Earnings</h2>
    <table><thead><tr><th>Type</th><th>Hours</th><th>Rate</th><th>Current</th><th>YTD</th></tr></thead><tbody>
      <?php foreach ($earnRows as $row): ?>
        <tr><td><?= htmlspecialchars($row['label']) ?></td><td style="text-align:right;"><?= isset($row['hours'])?number_format((float)$row['hours'],2):'' ?></td><td style="text-align:right;"><?= isset($row['rate'])?number_format((float)$row['rate'],2):'' ?></td><td style="text-align:right;"><?= number_format((float)$row['current'],2) ?></td><td style="text-align:right;">&dollar;<?= number_format((float)$row['ytd'],2) ?></td></tr>
      <?php endforeach; ?>
    </tbody></table>
  </div>
