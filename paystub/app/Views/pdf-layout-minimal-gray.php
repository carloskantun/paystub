<?php /** @var array $payload */ $tokens = $tokens ?? ['primary'=>'#111827','accent'=>'#6B7280','muted'=>'#6B7280']; ?>
<?php include __DIR__.'/partials/pdf-style-minimal-gray.php'; ?>
<div>
  <div class="block">
    <?php $i=$payload['stub_index']??0; $p=$payload['periods'][$i]??null; ?>
    <?php if (!empty($payload['branding']['logo'])): ?>
      <div style="margin:0 0 6pt;max-width:220pt;">
        <img src="<?= htmlspecialchars($payload['branding']['logo']) ?>" alt="Logo" style="max-width:100%;max-height:32pt;object-fit:contain;">
      </div>
    <?php else: ?>
      <h1>Pay Stub</h1>
    <?php endif; ?>
    <div class="muted" style="font-size:9pt;">Folio: <?= htmlspecialchars($payload['folio'] ?? '') ?> · Period: <?= htmlspecialchars($p['period_start'] ?? $p['start_date'] ?? '') ?> – <?= htmlspecialchars($p['period_end'] ?? $p['end_date'] ?? '') ?> · Pay: <?= htmlspecialchars($p['pay_date'] ?? '') ?></div>
  </div>
  <div class="block">
    <table><tbody><tr>
